...
class TestElements:
    class TestTextBox:
        ...
        def test_test_box(self, driver):  #  заполнение полей и создание пользователя
            test_box_page = TextBoxPage(driver, 'https://demoqa.com/text-box')   #  из класса бейспэйдж импортируем драйвер, открываем урл, открытие страницы в браузере
