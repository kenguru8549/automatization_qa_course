...
class BasePage:  #  создаем главный класс базовой страницы, другие бкдкт его наследовать
    ...
    def action_dgar_and_drop_to_element(self, what, where):  #  метод перемещения по странице из одной точки в другую
        action = ActionChains(self.driver)
        action.drag_and_drop(what, where)  #  параметры (откуда и куда)
        action.perform()  #  запускаем перемещение
